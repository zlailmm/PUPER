...
public:
    ...
    MultiSet(const MultiSet &ms){tid = ms.tid;sid = ms.sid;mapTokens = ms.mapTokens;color_count=ms.color_count;}//unfolding
...
//    const map<token,token_count_t,tokenLessthan> &getmapTokens(){return mapTokens;}
    ...
    //unf
    int tokennum_unf(){int sum=0;auto iter = mapTokens.begin();while(iter!=mapTokens.end()){sum+=iter->second;iter++;}return sum;}
};
