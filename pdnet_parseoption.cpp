...
//        {"-file",1},
        ...
        {"-damer",            0},//damer check
        {"-heur",             0},//unfolding heuristic
        {"-redu",             0},//synchronization reduction
        {"-showUnf",          0},//print unfolding
        {"-showETree",        0}//print explore tree
//        {"-ltlv",0},
//        {"-noout",0},
//        {"-directbuild",0},
//        {"-slice",0},
...
model_checking(string check_file, string property_file, LTLCategory ltltype, int num, bool gen_picture, bool showtree,
               ...
               bool damerON, string csvFileName_unf, bool hasHeur, bool hasRedu, bool showUnf, bool showETree) {
...
//            clock_t unfold_start=clock();
        ...
        synch->ba2cpn(ltltype, num, cpnet);
        synch->synch();
...
//        clock_gettime(CLOCK_REALTIME, &unfold_build_end);
        ...
        //synch reduction
        if(hasRedu) {
            Reduction *redu = new Reduction(synch);
            redu->reduce();
        }
        //Heuristic+
        Heuristic *heur=new Heuristic;
        if(hasHeur)
            heur->cal_Heuristic(synch,num, ltltype);
        clock_gettime(CLOCK_REALTIME, &unfold_build_end);

//        ProfilerStart("cpu.prof");
//        HeapProfilerStart("memory");
        auto mypid = getpid();
        string cmd = "grep VmSize /proc/" + to_string(mypid) + "/status";
        system(cmd.c_str());
        struct timespec unfold_begin;
        clock_gettime(CLOCK_REALTIME, &unfold_begin);
        UNFOLDING *unfold = new UNFOLDING;
        if(hasHeu)
            unfold->heurON=true;
        unfold->unfolding(synch,heur);
        if(showUnf)
            unfold->unfpdn->print_UNF("unfolding",unfold->cpn);
        if(showETree)
            unfold->print_ExploreTree("ExploreTree");
        cmd = "grep VmPeak /proc/" + to_string(mypid) + "/status";
        auto mem1=system(cmd.c_str());
//        HeapProfilerStop();
//        ProfilerStop();
//            clock_t unfold_finish=clock();
        ...
        auto check_time_only=get_timeSub(unfold_finish, unfold_begin);
        string unf_build_str, unf_total_str,unf_check_str;
        std::stringstream ss_ub, ss_ut,ss_uc;
        ...
        ss_uc << fixed << std::setprecision(3) << check_time_only;
        unf_check_str = ss_uc.str();
        ...
        cout << "unfold check time=" << setiosflags(ios::fixed) << setprecision(3) << check_time_only << endl;
        ...
        csvFormatAppend(csvOutputStr_unf, unf_check_str);
...
//            clock_t unfold_build_end=clock();
        ...
        //Heuristic+
        Heuristic *heur=new Heuristic;
        heur->cal_Heuristic(cpn_product,num, ltltype);

        UNFOLDING *unfold = new UNFOLDING;
        unfold->unfolding(cpn_product,heur);
//            clock_t unfold_finish=clock();
        struct timespec unfold_finish;
        clock_gettime(CLOCK_REALTIME, &unfold_finish);
        auto build_time_unf = get_timeSub(unfold_build_end, unfold_start);
...
void cmdlinet::doit() {
    ...
    try {
        ...
        bool showtree = false, showcpn = false, showtime = false,
                ...
                hasHeur=false, hasRedu=false, showUnf=false, showETree=false;
        ...
        if (opt_exist("-redu"))
            hasRedu = true;
        if (opt_exist("-showUnf"))
            showUnf = true;
        if (opt_exist("-showETree"))
            showETree = true;
...
//            cout << "time of program slice:"<<programSliceTime<<endl;
        ...
        if (!hasCIA) {
            string csvOutputStr = model_checking(filename, property_file, ltltype, fnum, showcpn, showtree, hasHeu,
                                                 ...
                                                 csvFileName_unf , hasHeur, hasRedu, showUnf, showETree);
...
void cmdlinet::help() {
    std::cout << "\n"
                 "* *              A concurrent program model checking tool PUPER              * *\n";
    std::cout <<
              //              "* *                        Chen Cheng                       * *\n"
              ...
              " -heur                   heuristic exploration\n"
              " -redu                   net reduction\n"
              " -showUnf                generate picture for unfolding\n"
              " -showETree              generate picture for ExploreTree\n";
