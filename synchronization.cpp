...
void SYNCH::add_copy_obs(index_t BA_t, set<index_t> obs_T) {
    for (auto i = obs_T.begin(); i != obs_T.end(); i++) {
        ...
        map_T2CP.emplace(make_pair(t->getid(), name));
...
//    cpn_synch->BA_tBeginIdx=ba_t_begin_idx;
    ...
    for (int i = 0; i < BA.get_vex_num(); i++) {
        ...
        if (vertics[i].accepted)
            state_accepted.insert(name);
        set_BAstate.emplace(cpn_synch->get_placecount() - 1);
        q_count++;
    }
    ...
    for (int i = 0; i < BA.get_vex_num(); i++) {
        ...
        for (BA_delta *p1 = vertics[i].firstarc; p1; p1 = p1->next) {
            string name;
            auto &p=cpn_synch->getplacearr()[map_state2place.find(p1->destination)->second];
            if (state_accepted.find(p.getid()) != state_accepted.end()) {
...
void SYNCH::synch() {
    ...
    for (auto i = map_t2predicates.begin(); i != map_t2predicates.end(); i++) {
        ...
        //保存t的guard
        if (!guard.empty()) {
            auto &t = cpn_synch->gettransarr()[i->first];
            map_t2guard.insert(make_pair(t.getid(), guard));
        }
        //连弧
