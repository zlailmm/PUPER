...
vector<String_t> merge_vec_unf(vector<String_t> vec1, vector<String_t> vec2) {
    ...
    //ba+
//    if (*vec1.begin() == "'ba_init'")
//        return vec2;
//    else if (count(vec2.begin(), vec2.end(), "'ba_init'"))
//        return vec1;
    //ba-
...
set<index_t> get_x(CTransition *t, CUT *cut, Binding_unf *binding, CPN *cpn,SYNCH *synch) {
    set<index_t> x;
...
//            x_e.insert(event[*i].read_varB.begin(),event[*i].read_varB.end());
            if (x == x_e) {
                if (is_readT) {
...
bool UNFOLDING::handleLTLF(string s, Config *C) {
    if (s[0] == '!') //前面带有'!'的is-fireable{}
    {
        ...
        while (1) {
            ...
            auto t_CP_id = synch->map_T2CP.find(subs)->second;
            auto t_CP_idx=cpn->mapTransition.find(t_CP_id)->second;
            vector<Binding_unf *> bindings;
            if (is_firable(t_CP_idx, C, bindings))
                return false;
...
bool UNFOLDING::is_firable(index_t idx, Config *C, vector<Binding_unf *> &bindings) {
    ...
    auto iter = synch->map_t2guard.find(t->getid());
...
//                first_call_enter= false;
            for (unsigned int j = 0; j < tran->get_producer().size(); j++) {
                ...
                for (unsigned int k = 0; k < place->get_producer().size(); k++) {
                    ...
                    if ((arcType == control || arcType == call_enter) && exist_in(searched[i].second.back(), t_id)) {
                        ...
                        if (cpn->map_t_call2thread.find(make_pair(t_idx, callStk)) != cpn->map_t_call2thread.end())
    ...
    for (auto i = call2T.second.begin(); i != call2T.second.end(); i++) {
        searched.emplace_back(*i, call2T.first);
    }
    //将search加入t_path
    bool enabled = false;
    ...
    for (index_t i = 0; i < cpn->get_transcount(); i++) {
        ...
        if (is_firable(i, C, bindings)) {
            for (auto binding = bindings.begin(); binding != bindings.end(); binding++) {
                ...
                pe->x = get_x(t, C->cut, *binding, cpn,synch);
...
//                break;
        ...
        e=heur->get_nextE(en_pe,C,unfpdn);
    }
...
void UNFOLDING::unfolding(SYNCH *synch,Heuristic *heur) {
    getsynch(synch);
    getheur(heur);
    init();
    Config *C = new Config;
...
//    print_ExploreTree("ExploreTree_ori");
