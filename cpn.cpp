...
string arr_suffix = "_arr",var_suffix = "_var",begin_suffix = " begin",
        id_suffix = "_id",tid_str = "__tid",//unfolding
...
//            statement = statement->parent;
        ...
        //unfolding
        if(currentRow!=p->row)
            map_NoneRow.emplace(control_T,p->row);
...
//            string statement_T = get_enter_T(statement_P)[0];
            ...
            //unfolding
            set_thread_enterT.insert(enter_T.begin(),enter_T.end());
            vector<string> beginP_endP;
            beginP_endP.emplace_back(func_begin_P);
            beginP_endP.emplace_back(func_end_P);
            map_thread2beginP_endP.emplace(enter_T, beginP_endP);
        }
...
//                thread_idexp = thread_v;
        ...
        else if(p->child->place == "pthread_mutex_lock")
        {
            ...
            //unfolding
            set<index_t> lockT;
            auto iter=map_lockT.find(mutex_v);
            auto t_row= getRow_unf(mapTransition.find(enter_T[0])->second);
            if(iter==map_lockT.end()){
                lockT.emplace(t_row);
                map_lockT.emplace(make_pair(mutex_v,lockT));
            } else{
                iter->second.emplace(t_row);
            }
        }
        else if(p->child->place == "pthread_mutex_unlock")
        {
            ...
            Add_Arc(enter_T[0],mutex_P,Exp,false,control,normal);
            //unfolding
            set_mutexP.emplace(mutex_P);
            set<index_t> unlockT;//行号
            auto iter=map_unlockT.find(mutex_v);
            auto t_row= getRow_unf(mapTransition.find(enter_T[0])->second);
            if(iter==map_unlockT.end()){
                unlockT.emplace(t_row);
                map_unlockT.emplace(make_pair(mutex_v,unlockT));
            } else{
                iter->second.emplace(t_row);
            }
        }
        ...
        else if(p->child->place == "pthread_cond_wait")
        {
            ...
            //unfolding
            set_mutexP.emplace(mutex_P);
            set_condP.emplace(cond_P);

            auto enter_t= findT_byid(enter_T[0]);
            map_NoneRow.emplace(T1,enter_t->getRow());
            map_NoneRow.emplace(T2,enter_t->getRow());
//            vector<string> cor_P;
//            cor_P.push_back(P1);
//            cor_P.push_back(P2);
//            cor_P.push_back(P3);
...
//        else
        ...
        //unfolding
        //记录call_enter和call_exit变迁,用于划分变迁所属线程
        map_call_enterP2exitP.emplace(call_P,statement_P);
        map_NoneRow.emplace(call_T,p->row);
...
void CPN::cal_t2thread_init() {
    //judge initialization done before
    if(!map_call_enterT2exitP.empty())
        return;

    //call_enterP2exitP ->call_enterT2exitP
    //call_enterP2exitP ->call_exitT2enterT
    for (auto i = map_call_enterP2exitP.begin(); i != map_call_enterP2exitP.end(); i++) {
        auto enter_T_idx = findP_byid(i->first)->get_consumer().front().idx;
        auto enter_T = findT_byindex(enter_T_idx)->getid();
        map_call_enterT2exitP.emplace(enter_T, i->second);

        auto exit_T_idx = findP_byid(i->second)->get_consumer().front().idx;
        auto exit_T = findT_byindex(exit_T_idx)->getid();
        map_call_exitT2enterT.emplace(exit_T, enter_T);
    }
    //main函数加入线程
    vector<string> main_beginP_endP;
    if (f_table.findfunc("main")) {
        main_beginP_endP.emplace_back(f_table.findbeginplace("main"));
        main_beginP_endP.emplace_back(f_table.findendplace("main"));
        vector<string> enterT;
        enterT.emplace_back(mainCall);
        map_thread2beginP_endP.emplace(enterT, main_beginP_endP);//main函数没有线程创建变迁，故只用一个string区别它与其他线程
    }
}

void CPN::cal_t2thread() {
    cal_t2thread_init();

    for (auto ithread = map_thread2beginP_endP.begin(); ithread != map_thread2beginP_endP.end(); ithread++) {
        vector<CTransition *> searched;
        set<index_t> endT;
        //计算开始变迁
        auto beginP = findP_byid(ithread->second.front());
        for (auto ibeginT = beginP->get_consumer().begin(); ibeginT != beginP->get_consumer().end(); ibeginT++) {
            auto beginT = findT_byindex(ibeginT->idx);
            searched.emplace_back(beginT);
        }

        //计算结束变迁
        auto endP = findP_byid(ithread->second.back());
        for (auto iendT = endP->get_producer().begin(); iendT != endP->get_producer().end(); iendT++) {
            endT.emplace(iendT->idx);
        }

        type_callStack call_stack;
        call_stack.emplace_back(ithread->first);
        for (unsigned int i = 0; i < searched.size(); i++) {
            CTransition *tran = searched[i];
            auto idx = mapTransition.find(tran->getid())->second;
            if (endT.find(idx) != endT.end()) {
                for (unsigned int j = 0; j < tran->get_consumer().size(); j++) {
                    auto arcType = tran->get_consumer()[j].arcType;
                    CPlace *place = &getplacearr()[tran->get_consumer()[j].idx];
                    if (arcType == readArc || arcType == writeArc || arcType == readwrite
                        || arcType == allocwrite || arcType == remain) {
                        set_varP_normal.emplace(tran->get_consumer()[j].idx);
                    }
                }
                continue;
            }
            for (unsigned int j = 0; j < tran->get_consumer().size(); j++) {
                ...
                if (arcType == executed || arcType==call_exit) {
                    for (unsigned int k = 0; k < place->get_consumer().size(); k++) {
                        if (!exist_in(searched, &gettransarr()[place->get_consumer()[k].idx]))
                            searched.emplace_back(&gettransarr()[place->get_consumer()[k].idx]);
                    }
                }
                    //call_enter如果是创建线程不需要探索;
                else if(arcType==call_enter){
                    //将func内的变迁与call变迁组合，区分不同call内的func变迁;
                    //返回退出call变迁，加入searched;
                    auto place_end=cal_t2thread_call(ithread->first.front(),tran->getid(),place,call_stack);
                    for (unsigned int k = 0; k < place_end->get_consumer().size(); k++) {
                        if (!exist_in(searched, &gettransarr()[place_end->get_consumer()[k].idx]))
                            searched.emplace_back(&gettransarr()[place_end->get_consumer()[k].idx]);
                    }
                }
                else if (arcType == readArc || arcType == writeArc || arcType == readwrite
                         || arcType == allocwrite || arcType == remain) {
                    set_varP_normal.emplace(tran->get_consumer()[j].idx);
                }
            }
...
//            //排除线程创建变迁
//            if(set_thread_enterT.find((*i)->getid())!=set_thread_enterT.end())
//                continue;
            auto t_idx = mapTransition.find((*i)->getid())->second;
            map_t_call2thread.emplace(make_pair(t_idx,call_stack), ithread->first[0]);
        }
    }
}

CPlace* CPN::cal_t2thread_call(string thread,string callT,CPlace *place ,type_callStack call_stack) {
    vector<CTransition *> searched;
    string endP_id;
    set<index_t >endT;
    vector<string> callTs;
    callTs.emplace_back(callT);
    call_stack.emplace_back(callTs);

    for(auto ibeginT=place->get_consumer().begin();ibeginT!=place->get_consumer().end();ibeginT++){
        auto beginT= findT_byindex(ibeginT->idx);
        searched.emplace_back(beginT);
    }

    auto f_map=f_table.get_f_map();
    for(auto i=f_map.begin();i!=f_map.end();i++){
        if(i->second.begin_place==place->getid()){
            endP_id=i->second.end_place;
            break;
        }
    }
    auto exitP_id=map_call_enterT2exitP.find(callT)->second;
    auto endP= findP_byid(endP_id);
    auto exitP= findP_byid(exitP_id);
    for(auto iendT=endP->get_producer().begin();iendT!=endP->get_producer().end();iendT++){
        endT.emplace(iendT->idx);
    }
    for(auto iexitT=exitP->get_producer().begin();iexitT!=exitP->get_producer().end();iexitT++){
        endT.emplace(iexitT->idx);
    }
    for(unsigned int i=0;i<searched.size();i++) {
        CTransition *tran = searched[i];
        auto idx = mapTransition.find(tran->getid())->second;
        ...
        for (unsigned int j = 0; j < tran->get_consumer().size(); j++) {
            ...
            if (arcType == executed ||arcType==call_exit) {
                ...
                //返回退出call变迁，加入searched;
                CPlace *place = &getplacearr()[tran->get_consumer()[j].idx];
                auto place_end=cal_t2thread_call(thread,tran->getid(),place,call_stack);
                for (unsigned int k = 0; k < place_end->get_consumer().size(); k++) {
                    if (!exist_in(searched, &gettransarr()[place_end->get_consumer()[k].idx]))
                        searched.emplace_back(&gettransarr()[place_end->get_consumer()[k].idx]);
                }
    ...
    for (auto i = searched.begin(); i != searched.end(); i++) {
        ...
        if(set_thread_enterT.find((*i)->getid())!=set_thread_enterT.end())
            continue;
//        //排除BA变迁
//        auto idx= mapTransition.find((*i)->getid())->second;
//        if(idx>=BA_tBeginIdx)
//            continue;

        auto t_idx = mapTransition.find((*i)->getid())->second;
        map_t_call2thread.emplace(make_pair(t_idx,call_stack), thread);
    }
    return exitP;
}

void CPN::cal_conflictT_init() {
    //配对lock和unlock变迁
    for(auto ilock_v=map_lockT.begin();ilock_v!=map_lockT.end();ilock_v++){
        auto iunlock_v=map_unlockT.find(ilock_v->first);
        auto i=ilock_v->second.begin();
        auto j=iunlock_v->second.begin();
        map<index_t,index_t > map_lk2ulk;
        for(;i!=ilock_v->second.end();i++,j++){
            map_lk2ulk.emplace(*i,*j);
        }
        map_mutex_range.emplace(ilock_v->first,map_lk2ulk);
    }
    //记录函数第一行，不计算其冲突，因为在调用处已经计算过冲突;
    auto f_map=f_table.get_f_map();
    for(auto i=f_map.begin();i!=f_map.end();i++){
        //若库所被约简，则跳过
        if(mapPlace.find(i->second.begin_place)==mapPlace.end())
            continue;

        auto t_idx=findP_byid(i->second.begin_place)->get_consumer().front().idx;
        set_func_beginT.emplace(t_idx);
    }
}

void CPN::cal_conflictT() {
    cal_conflictT_init();
    //mutex
    for(auto i=set_mutexP.begin();i!=set_mutexP.end();i++){
        auto p_idx=mapPlace.find(*i)->second;
        cal_conflictT_step(p_idx,mutex);
    }
    //conditon
    for(auto i=set_condP.begin();i!=set_condP.end();i++){
        auto p_idx=mapPlace.find(*i)->second;
        cal_conflictT_step(p_idx,condition);
    }
    //normal var
    for(auto i=set_varP_normal.begin();i!=set_varP_normal.end();i++){
        cal_conflictT_step(*i,read_write);
    }
}

void CPN::cal_conflictT_step(index_t p_idx,Var_Type var_type) {
    CPlace *p= findP_byindex(p_idx);
    switch (var_type) {
        case mutex: {
            vector<type_T_Call2thread> readT, writeT;
            for (auto i = p->get_consumer().begin(); i != p->get_consumer().end(); i++) {
                //排除函数参数行（第一行）;
                if(set_func_beginT.find(i->idx)!=set_func_beginT.end())
                    continue;

                for(auto j=map_t_call2thread.begin();j!=map_t_call2thread.end();j++){
                    if(j->first.first==i->idx)
                        writeT.emplace_back(j->first,j->second);
                }
            }
            cal_conflictT_byRW(readT, writeT);
            break;
        }
        case condition: {
            vector<type_T_Call2thread> readT, writeT;
        ...
        case read_write:{
            vector<type_T_Call2thread> readT, writeT;
            for (auto i = p->get_producer().begin(); i != p->get_producer().end(); i++) {
                //排除函数参数行（第一行）;
                if(set_func_beginT.find(i->idx)!=set_func_beginT.end())
                    continue;

                if (i->arcType == readArc) {
                    for(auto j=map_t_call2thread.begin();j!=map_t_call2thread.end();j++){
                        if(j->first.first==i->idx) {
                            readT.emplace_back(j->first, j->second);

                            auto ireadT=map_readT.find(j->first.first);
                            if(ireadT!=map_readT.end()) {
                                ireadT->second.emplace(p_idx);
                            } else {
                                set<index_t> set_p;
                                set_p.emplace(p_idx);
                                map_readT.emplace(j->first.first,set_p);
                            }
                        }
                    }
                } else {
                    //用于处理赋值语句
                    auto iter=map_p2writeT.find(p_idx);
                    if(iter!=map_p2writeT.end()){
                        iter->second.emplace(i->idx);
                    } else {
                        set<index_t > writeT;
                        writeT.emplace(i->idx);
                        map_p2writeT.emplace(p_idx,writeT);
                    }
...
//            handle_within_Lock_unLock(readT);
//            handle_within_Lock_unLock(writeT);
            cal_conflictT_byRW(readT, writeT);
        }
    }
}

bool CPN::is_create_thread(std::string P_id) {
    for(auto i=map_thread2beginP_endP.begin();i!=map_thread2beginP_endP.end();i++){
        if(i->second.front()==P_id)
            return true;
    }
    return false;
}

void CPN::map_t2conflictT_add(vector<type_T_Call2thread> trans, vector<type_multimapCall2T> Call2Ts) {
    for (auto i = 0; i < trans.size(); i++) {
        //t-->map(call,conflictT)
        auto t2call=trans[i].first;//原变迁
        auto i_t = map_t2conflictT.find(t2call);
        if (i_t == map_t2conflictT.end()) {
            if (!Call2Ts[i].empty())
                map_t2conflictT.emplace(t2call, Call2Ts[i]);
        } else {
            //call-->T
            for (auto iCall2T = Call2Ts[i].begin(); iCall2T != Call2Ts[i].end(); iCall2T++) {
                auto iter = i_t->second.begin();
                for(;iter!=i_t->second.end();iter++){
                    if(iter->first==iCall2T->first){
                        if(*iter->second.begin()==*iCall2T->second.begin())
                            break;
                    }
                }
                if (iter == i_t->second.end()) {
                    i_t->second.emplace(*iCall2T);
                }
            }
        }
    }
...
void CPN::handle_within_Lock_unLock(vector<type_T_Call2thread> &Ts) {
//    for(auto i=Ts.begin();i!=Ts.end();){
//        auto t= findT_byindex(i->first.first);
//        auto row=t->getRow();
//        //转化无行号语句
//        if(row==65535){
//            row=map_NoneRow.find(t->getid())->second;
//        }
//        auto j=map_lock_unlock.begin();
//        for(;j!=map_lock_unlock.end();j++){
//            if(row>j->first &&row<j->second) {
//                break;
//            }
//        }
//        if(j==map_lock_unlock.end())
//            i++;
//        else {
//            i=Ts.erase(i);
//        }
...
bool same_criticalSec(index_t row1,index_t row2,map<index_t,index_t> map_range){
    bool within1=false,within2=false;
    for(auto i=map_range.begin();i!=map_range.end();i++){
        if(row1>i->first && row1<i->second)
            within1= true;
        if(row2>i->first && row2<i->second)
            within2= true;
    }
    if(within1 && within2)
        return true;
...
bool CPN::within_criticalSec(type_T2Call tCall1,type_T2Call tCall2) {
    set<index_t > row_set1,row_set2;

    auto t1= findT_byindex(tCall1.first);
    auto t2= findT_byindex(tCall2.first);
    auto row1=getRow_unf(tCall1.first);
    auto row2=getRow_unf(tCall2.first);
    row_set1.emplace(row1);
    row_set2.emplace(row2);

    for(auto i=tCall1.second.begin()+1;i!=tCall1.second.end();i++){//begin是线程创建变迁，跳过
        auto t= findT_byid(i->front());
        auto row=getRow_unf(mapTransition.find(i->front())->second);
        row_set1.emplace(row);
    }
    for(auto i=tCall2.second.begin()+1;i!=tCall2.second.end();i++){//begin是线程创建变迁，跳过
        auto t= findT_byid(i->front());
        auto row= getRow_unf(mapTransition.find(i->front())->second);
        row_set2.emplace(row);
    }

    for(auto i=map_mutex_range.begin();i!=map_mutex_range.end();i++){
        for(auto irow1=row_set1.begin();irow1!=row_set1.end();irow1++) {
            for(auto irow2=row_set2.begin();irow2!=row_set2.end();irow2++) {
                if (same_criticalSec(*irow1, *irow2, i->second))
                    return true;
            }
        }
    }
...
void CPN::cal_conflictT_byRW(vector<type_T_Call2thread> readT, vector<type_T_Call2thread> writeT) {
    vector<type_multimapCall2T> conflictT_r(readT.size());//t --> map(row,set<conflictT>)
    vector<type_multimapCall2T> conflictT_w(writeT.size());
    //readT-->writeT
    for (unsigned int i = 0; i < readT.size(); i++) {
        for (unsigned int j = 0; j < writeT.size(); j++) {
            if (readT[i].second != writeT[j].second) {//不同线程
                if(within_criticalSec(readT[i].first,writeT[j].first))
                    continue;
                //查找conflictT_r[i]中是否存在Call;接着查找Row，将Row相同的变迁放入同一集合;
                conflictT_add(conflictT_r[i],writeT[j]);
                conflictT_add(conflictT_w[j],readT[i]);
            }
        }
    }
    //writeT-->writeT
    for (unsigned int i = 0; i < writeT.size(); i++) {
        for (unsigned int j = i + 1; j < writeT.size(); j++) {
            if (writeT[i].second!=writeT[j].second) {//不同线程
                if(within_criticalSec(writeT[i].first,writeT[j].first))
                    continue;
                //查找conflictT_r[i]中是否存在Call;接着查找Row，将Row相同的变迁放入同一集合;
                conflictT_add(conflictT_w[i],writeT[j]);
                conflictT_add(conflictT_w[j],writeT[i]);
            }
        }
    }
    //readT加入map_t2conflictT
    map_t2conflictT_add(readT, conflictT_r);
    //writeT加入map_t2conflictT
    map_t2conflictT_add(writeT, conflictT_w);
}

void CPN::conflictT_add(type_multimapCall2T &mapCall2T, type_T_Call2thread t_call2thread) {
    //查找conflictT_r[i]中是否存在Call;接着查找Row，将Row、Call相同的变迁放入同一集合;
    auto iter1=mapCall2T.begin();
    for(;iter1!=mapCall2T.end();iter1++){
        if(iter1->first == t_call2thread.first.second) {//Call相同
            auto idx1=*iter1->second.begin();
            auto idx2=t_call2thread.first.first;
            auto row1= getRow_unf(idx1);
            auto row2= getRow_unf(idx2);
            if(row1==row2){
                iter1->second.emplace(t_call2thread.first.first);
                break;
            }
        }
    }
    //不存在ROW、Call相同的变迁
    if(iter1==mapCall2T.end()){
        set<index_t >conflictT;
        conflictT.emplace(t_call2thread.first.first);
        mapCall2T.emplace(t_call2thread.first.second,conflictT);
    }
}

bool CPN::is_varP(index_t P_idx){
    auto P_id= findP_byindex(P_idx)->getid();
    if(set_mutexP.find(P_id)!=set_mutexP.end())
        return true;
    if(set_condP.find(P_id)!=set_condP.end())
        return true;
    if(set_varP_normal.find(P_idx)!=set_varP_normal.end())
        return true;
    return false;
}

bool CPN::is_readT(index_t T_idx,set<index_t> &readP){
    auto iter=map_readT.find(T_idx);
    if(iter==map_readT.end())
        return false;
...
unsigned short CPN::getRow_unf(index_t t_idx){
    auto t= findT_byindex(t_idx);
    auto iter=map_NoneRow.find(t->getid());
    if(iter==map_NoneRow.end()){
        return t->getRow();
    }
    return iter->second;
}
