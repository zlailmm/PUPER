...
//unfolding
typedef vector<string> type_call;//线程创建变迁可以有两个，因此采用向量vector<string>存储调用变迁。
typedef vector<type_call> type_callStack;//按调用先后压入栈
typedef multimap<type_callStack,set<index_t>> type_multimapCall2T;
typedef pair<type_callStack ,set<index_t>> type_Call2T;
typedef pair<index_t ,type_callStack> type_T2Call;
typedef pair<type_T2Call ,string> type_T_Call2thread;//T和call绑定，区分不同调用中的变迁;
enum Var_Type{read_write,mutex,condition};
const string mainCall="&main";
...
//    }
    ...
    //unfolding
    bool has_token(){return getMultiSet().tokennum_unf();};
} CPlace;
...
//    }
    //synch reduction
    void change_src_tgt(string source,string target){source_id = source;target_id = target;};
} CArc;
...
public:
    ...
    CPN_Place* findP_byindex(index_t index){if(placecount<=index){
        cout<< "index out of placecount!"<<endl;
        exit(-1);}return &place[index];}
    ...
    CPN_Transition* findT_byindex(index_t index){
        if(transitioncount<=index)
            throw "index out of transcount!";
    ...
    //unfolding
    set<string > set_thread_enterT;//创建线程变迁
    map<vector<string>,vector<string>> map_thread2beginP_endP;
//    map<string,set<index_t>> map_thread2t;
//    map<index_t ,string> map_t2thread;
    map<type_T2Call ,string> map_t_call2thread;//func中的变迁需要根据call变迁区分。
    set<string > set_mutexP;//mutex
    set<string > set_condP;//条件变量
    set<index_t >set_varP_normal;//其余变量（线程中的）
    map<index_t,set<index_t> >map_readT;//读变迁->被读变量库所
    map<type_T2Call ,type_multimapCall2T> map_t2conflictT;//t,call --> map(conflictCall,set<conflictT>)
//    NUM_t BA_tBeginIdx;
    void unf_init();//cex相关信息计算
    void cal_t2thread();//各线程对应的变迁
    CPlace *cal_t2thread_call(string thread,string callT,CPlace *place ,type_callStack call_stack);
    void cal_conflictT();
    void cal_conflictT_step(index_t p_idx,Var_Type var_type);

    //unfolding Auxiliary
    void cal_t2thread_init();
    void cal_conflictT_init();
    map<string ,set<index_t >> map_lockT;//map<var,行号>
    map<string ,set<index_t >> map_unlockT;//map<var,行号>
    map<string ,map<index_t ,index_t> > map_mutex_range;//map<var,map<行号,行号>>
    map<string,string> map_call_enterT2exitP;//函数调用语句的入口T和返回P,即在函数begin的前一个变迁和函数end绑定的v库所;（一个call对应一个v）
    map<string,string> map_call_enterP2exitP;//中间映射，由于弧未连上，因此暂时保存P2P，用于后续计算上方的映射T2P;
    map<string,string> map_call_exitT2enterT;
    set<index_t> set_func_beginT;
    map<string,index_t > map_NoneRow;//复合语句行号；<tid,row>

    bool is_create_thread(string P_id);
    void handle_within_Lock_unLock(vector<type_T_Call2thread> &Ts);
    bool within_criticalSec(type_T2Call tCall1,type_T2Call tCall2);
    void cal_conflictT_byRW(vector<type_T_Call2thread> readT,vector<type_T_Call2thread> writeT);//通过读写T求conflictT;cal_conflictT_step的子函数;
    void map_t2conflictT_add(vector<type_T_Call2thread> trans,vector<type_multimapCall2T> Call2Ts);
    void conflictT_add(type_multimapCall2T &conflictT,type_T_Call2thread t2thread_Call);
    bool is_varP(index_t P_idx);
    bool is_readT(index_t T_idx,set<index_t> &readP);

    unsigned short getRow_unf(index_t t_idx);


    //unfolding heuristic
    map<index_t ,set<index_t> > map_p2writeT;//变量库所2写该变量的变迁；
...
//unf heur
    //guard
    map<string,string > map_v2place;
    Variable *get_vartable(){return vartable;};
